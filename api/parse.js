export default async function handler(req, res) {
  ...
  if (!extra || typeof extra !== 'string') {
    return res.status(200).json({ genres: [], moods: [], keywords: [], keywords_en: [] });
  }
...
- "우울해","형 우울해","지쳤어","힐링 필요","위로받고 싶어" → moods: ["편안한","감동적인"], genres: ["치유","드라마"], keywords: ["힐링","위로","따뜻한"], keywords_en: ["healing","comfort","emotional"]
- "이세계물","환생물","전생물","회귀물","빙의물" → genres: ["이세계","환생","전생"], keywords: ["이세계","환생","전생","먼치킨"], keywords_en: ["isekai","reincarnation"]
...
예시 입력: 기분이 우울해
예시 출력:
{"moods":["편안한","감동적인"],"genres":["치유","드라마"],"keywords":["힐링","위로","따뜻한"],"keywords_en":["healing","comfort","emotional"]}
입력: ${extra}`;
  try {
    ...
    if (!response.ok) return res.status(200).json({ genres: [], moods: [], keywords: [], keywords_en: [] });
    const data = await response.json();
    const msg = (data.output || []).find(o => o.type === 'message');
    const text = msg?.content?.find(c => c.type === 'output_text')?.text || '{}';
    const jsonMatch = text.match(/\{[\s\S]*\}/);
    if (!jsonMatch) return res.status(200).json({ genres: [], moods: [], keywords: [], keywords_en: [] });
